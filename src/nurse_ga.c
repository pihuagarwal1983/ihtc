...
int admitPatientsNewGA(int** room_gender_map, PriorityQueue* pq, int* chromosome, int size) {
    ...
    for (int p = 0; p < size; ++p) {
        ...
        while (patients[p_id].is_admitted == 0) {
            d = patients[p_id].surgery_release_day + rand() % (patients[p_id].surgery_due_day - patients[p_id].surgery_release_day + 1);
            //a + rand() % (b - a + 1);
...
const int NURSE_POPULATION_SIZE, NURSE_NUM_ITER, NURSE_CONVERGENCE_STOPPING_CRITERION;
...
void applyNursesGA(void)
{
    ...
    for (i = 0; i < NURSE_POPULATION_SIZE; ++i) {
        resetValuesNurseGA();
        NURSE_POPULATION[i][CHROMOSOME_SIZE_NURSE_GA] = evaluateFitnessNurseGA(NURSE_POPULATION[i]);
    }
    //printf("%d\n", POPULATION[0]);
    ...
    for (i = 0; i <= NURSE_NUM_ITER; ++i) {
        if ((rand() / (float)RAND_MAX) <= p_c) {
            ...
            for (j = 0; j < 2; ++j)
                NURSE_CROSSOVER_OFFSPRING_STORAGE_PLACE[j][CHROMOSOME_SIZE_NURSE_GA] = evaluateFitnessNurseGA(NURSE_CROSSOVER_OFFSPRING_STORAGE_PLACE[j]);
            ...
            NURSE_MUTATED_OFFSPRING_STORAGE_PLACE[CHROMOSOME_SIZE_NURSE_GA] = evaluateFitnessNurseGA(NURSE_MUTATED_OFFSPRING_STORAGE_PLACE);
...
{
    int n_id, r_id, r_index, fitness = 0, nurse_shift, n_index, p_index, sh;
    bool track_nurse_shift[num_rooms][days*3] = {false};
    Patient* p;
    int most_suitable_nurse, most_suitable_nurse_shift;

    for (n_index = 0; n_index < CHROMOSOME_SIZE_NURSE_GA; ++n_index) {
        n_id = nurse_chromosome[n_index];
        for (int s = 0; s < nurses[n_id].num_shifts; s++) {
            for (r_index=0; r_index < num_rooms; ++r_index) {
                r_id = room[r_index];
                nurse_shift = (nurses[n_id].shift[s].day * 3) + nurses[n_id].shift[s].shift_time;
                if (track_nurse_shift[r_id][nurse_shift]) continue;
                if (nurses[n_id].shift[s].load_left < rooms_requirement[r_id][nurse_shift]->load_sum) continue;
                track_nurse_shift[r_id][nurse_shift] = true;

                if (nurses[n_id].shift[s].num_rooms == 0)
                    nurses[n_id].shift[s].rooms = (int *) calloc (++nurses[n_id].shift[s].num_rooms, sizeof(int))
                else
                    nurses[n_id].shift[s].rooms = (int *) realloc (nurses[n_id].shift[s].rooms, ++nurses[n_id].shift[s].num_rooms * sizeof(int));
                if (!nurses[n_id].shift[s].rooms)
                    ASSERT(0, "Dynamic Memory Allocation Error!");
                nurses[n_id].shift[s].rooms[nurses[n_id].shift[s].num_rooms-1] = r_id;

                if (room[r_id].length_of_nurses_alloted == 0)
                    room[r_id].nurses_alloted = (int *) calloc (++room[r_id].length_of_nurses_alloted, sizeof(int))
                else
                    room[r_id].nurses_alloted = (int *) realloc (room[r_id].nurses_alloted, ++room[r_id].length_of_nurses_alloted * sizeof(int));
                if (!room[r_id].nurses_alloted)
                    ASSERT(0, "Dynamic Memory Allocation Error!");
                room[r_id].nurses_alloted[room[r_id].length_of_nurses_alloted-1] = n_id;

                nurses[n_id].shift[s].load_left -= rooms_requirement[r_id][nurse_shift]->load_sum;

                for (p_index = 0; p_index < size_of_room_schedule[r_id][nurses[n_id].shift[s].day]; ++p_index) {
                    if (*room_schedule[r_id][nurses[n_id].shift[s].day][p_index] == 'p') {
                        p = str2int(room_schedule[r_id][nurses[n_id].shift[s].day][p_index]);
                        if (patients[p].num_nurses_allotted == 0)
                            patients[p].nurses_allotted = (int*)calloc(++patients[p].num_nurses_allotted, sizeof(int));
                        else
                            patients[p].nurses_allotted = (int*)realloc(patients[p].nurses_allotted, ++patients[p].num_nurses_allotted * sizeof(int));
                        if (!patients[p].nurses_allotted)
                            ASSERT(0, "Dynamic Memory Allocation Error!");
                        patients[p].nurses_allotted[patients[p].num_nurses_allotted - 1] = n_id;
                    }
                    else if (*room_schedule[r_id][nurses[n_id].shift[s].day][p_index] == 'a') {
                        occ = str2int(room_schedule[r_id][nurses[n_id].shift[s].day][p_index]);
                        if (occupants[occ].num_nurses_allotted == 0)
                            occupants[occ].nurses_allotted = (int*)calloc(++occupants[occ].num_nurses_allotted, sizeof(int));
                        else
                            occupants[occ].nurses_allotted = (int*)realloc(occupants[occ].nurses_allotted, ++occupants[occ].num_nurses_allotted * sizeof(int));
                        if (!occupants[occ].nurses_allotted)
                            ASSERT(0, "Dynamic Memory Allocation Error!");
                        occupants[occ].nurses_allotted[occupants[occ].num_nurses_allotted - 1] = n_id;
                    }
                }
            }
        }
    ...
    for (r_index = 0; r_index < num_rooms; ++r_index) {
        for (sh=0; sh<days*3; ++sh) {
            if (track_nurse_shift[r_index][sh] == false) {
                most_suitable_nurse = *nurse_chromosome;
                for (n_index = 1; n_index < num_nurses; ++n_index) {
                    n_id = nurse_chromosome[n_index];
                    for (int s = 0; s < nurses[n_id].num_shifts; s++) {
						if ((nurses[n_id].shift[s].day * 3) + nurses[n_id].shift[s].shift_time != sh) continue;
                        if (nurses[n_id].shift[s].load_left > nurses[most_suitable_nurse].shift[s].load_left) {
                            most_suitable_nurse = n_id;
                            most_suitable_nurse_shift = s;
                            continue;
                        }
                    }
                }

                n_id = most_suitable_nurse;
                s = most_suitable_nurse_shift;
                if (nurses[n_id].shift[s].num_rooms == 0)
                    nurses[n_id].shift[s].rooms = (int*)calloc(++nurses[n_id].shift[s].num_rooms, sizeof(int))
                else
                    nurses[n_id].shift[s].rooms = (int*)realloc(nurses[n_id].shift[s].rooms, ++nurses[n_id].shift[s].num_rooms * sizeof(int));
                if (!nurses[n_id].shift[s].rooms)
                    ASSERT(0, "Dynamic Memory Allocation Error!");
                nurses[n_id].shift[s].rooms[nurses[n_id].shift[s].num_rooms - 1] = r_index;

                if (room[r_index].length_of_nurses_alloted == 0)
                    room[r_index].nurses_alloted = (int*)calloc(++room[r_index].length_of_nurses_alloted, sizeof(int))
                else
                    room[r_index].nurses_alloted = (int*)realloc(room[r_index].nurses_alloted, ++room[r_index].length_of_nurses_alloted * sizeof(int));
                if (!room[r_index].nurses_alloted)
                    ASSERT(0, "Dynamic Memory Allocation Error!");
                room[r_index].nurses_alloted[room[r_index].length_of_nurses_alloted - 1] = n_id;

                nurses[n_id].shift[s].load_left -= rooms_requirement[r_index][s]->load_sum;
...
void resetValuesNurseGA(void) {
	...
	for (i = 0; i < num_occupants; i++)
		for (j = 0; j < occupants[i].num_nurses_allotted; ++j) {
			occupants[i].nurses_allotted[j] = -1;
...
void orderCrossoverNurseGA(void) {
    ...
    int max = 0;
    for (int i = 0; i < CHROMOSOME_SIZE_NURSE_GA; i++) {
        if (max < NURSE_CROSSOVER_PARENT_STORAGE_PLACE[0][i])
            max = NURSE_CROSSOVER_PARENT_STORAGE_PLACE[0][i];
    }
    ...
    for (i = r1; i <= r2; i++) {
        int gene1 = NURSE_CROSSOVER_PARENT_STORAGE_PLACE[0][i];
        int gene2 = NURSE_CROSSOVER_PARENT_STORAGE_PLACE[1][i];

        NURSE_CROSSOVER_OFFSPRING_STORAGE_PLACE[0][i] = gene1;
        NURSE_CROSSOVER_OFFSPRING_STORAGE_PLACE[1][i] = gene2;
    ...
    for (i = 0; i < CHROMOSOME_SIZE_NURSE_GA; i++) {
        ...
        int gene1 = NURSE_CROSSOVER_PARENT_STORAGE_PLACE[0][index];
        if (gene1 >= 0 && !visited1[gene1]) {
            while (NURSE_CROSSOVER_OFFSPRING_STORAGE_PLACE[1][k] != -1) {
                k = (k + 1) % CHROMOSOME_SIZE_NURSE_GA;
            }
            NURSE_CROSSOVER_OFFSPRING_STORAGE_PLACE[1][k] = gene1;
            visited1[gene1] = true;
        }

        int gene2 = CROSSOVER_PARENT_STORAGE_PLACE[1][index];
        if (gene2 >= 0 && !visited2[gene2]) {
            while (NURSE_CROSSOVER_OFFSPRING_STORAGE_PLACE[0][m] != -1) {
                m = (m + 1) % CHROMOSOME_SIZE_NURSE_GA;
            }
            NURSE_CROSSOVER_OFFSPRING_STORAGE_PLACE[0][m] = gene2;
            visited2[gene2] = true;
        }
    }
