...
const int HILL_CLIMBING_ITER = 1000000, SAME_COST_ITER = 1000, NUM_COST_ITER = 1000000;
...
void append_optionalsNewGA(void) {
    ...
    G_BEST[num_patients] = a;
    G_BEST[num_patients + 1] = b;

    for (int i = 0; i < optional_count; i++)
        G_BEST[i + mandatory_count] = optional_patients[i]->id;
}
...
// faulty implementation - don't run
//void ApplyHillClimbing(PriorityQueue* pq) {
...
void reinitializeOptionals(void) {
    int* temp_optional_patients = (int*)calloc(optional_count, sizeof(int));
    int r1, r2, temp;
    for (int i = 0; i < optional_count; i++)
        temp_optional_patients[i] = optional_patients[i]->id;

    for (i = 0; i < 100; ++i) {
        // swap patinets of "temp_optional_patients" internally 100 times
        do {
            r1 = rand() % optional_count;
            r2 = rand() % optional_count;
        } while (r1 == r2);
        temp_optional_patients[r1] += temp_optional_patients[r2];
        temp_optional_patients[r2] = temp_optional_patients[r1] - temp_optional_patients[r2];
        temp_optional_patients[r1] -= temp_optional_patients[r2];
    }
    
    int a = G_BEST[num_patients];
    int b = G_BEST[num_patients + 1];
    
    for (int i = 0; i < optional_count; i++)
        G_BEST[i + mandatory_count] = temp_optional_patients[i];
    
    // no need for this but doing just to be sure that the fitnesses are not overwritten
    G_BEST[num_patients] = a;
    G_BEST[num_patients + 1] = b;

    free(temp_optional_patients);
}

void ApplyNewHillClimbing(void) {
    int i = 0, same_cost_iter = 0;
    int* temp_solution = (int*)calloc((num_patients + 2), sizeof(int));
    if (!temp_solution) {
        printf("Memory allocation failed.\n");
        exit(EXIT_FAILURE);
    ...
    for (i = 0; i < HILL_CLIMBING_ITER; ++i) {
        if (same_cost_iter == SAME_COST_ITER) {
            reinitializeOptionals(); // this function shuffles the optional patients and appends them at the end of the mandatory ones in the G_BEST
            same_cost_iter = 0;
        }
        swapMutationoptionals(temp_solution); // check this function
        evaluateFitnessScoreNewGA(temp_solution, pq, num_patients);
        same_cost_iter++;
        if ((!temp_solution[num_patients]) && (temp_solution[num_patients + 1] < G_BEST[num_patients + 1])) {
            memcpy(G_BEST, temp_solution, (num_patients + 2) * sizeof(int));
            same_cost_iter = 0;
            continue;
        }
    ...
    printf("\nG_BEST : \n");
    for (int i = 0; i < num_patients + 2; i++)
        printf("%d\t", G_BEST[i]);
    printf("\nFinal feasible G_BEST after swap mutation applied.");
    printf("\nLast improvement from hill climbing: %d", last_improv);
    printf("\nNumber of improvements: %d", no_of_improv);
...
void applyNursesGA(void)
{
    ...
    float p_c = 0.8; // try with 0.6, 0.7, 0.8, 0.9
    ...
    memcpy(N_G_BEST, NURSE_POPULATION[0], (CHROMOSOME_SIZE_NURSE_GA + 1) * sizeof(int));
    N_G_BEST[CHROMOSOME_SIZE_NURSE_GA] = -1;
        ...
        if ((rand() / (float)RAND_MAX) <= p_c) {
            ...
            for (j = 0; j < 2; ++j)
                if (NURSE_CROSSOVER_OFFSPRING_STORAGE_PLACE[j][CHROMOSOME_SIZE_NURSE_GA] < N_G_BEST[CHROMOSOME_SIZE_NURSE_GA])
                    printf("\nImprovement (crossover) @ iter: %d", i);
                    memcpy(N_G_BEST, NURSE_CROSSOVER_OFFSPRING_STORAGE_PLACE[j], sizeof(int) * (CHROMOSOME_SIZE_NURSE_GA + 1));
        }
        else {
...
void resetValuesNurseGA(void) {
    int i, j;
    /*
    Chnages to be reset in resetValuesNurseGA function -
    -1. nurses[n_id].shift[s].load_left
    -2. nurses[n_id].shift[s].rooms
    -3. nurses[n_id].shift[s].num_rooms++
    -4. room[r_index].nurses_alloted
    -5. room[r_index].length_of_nurses_alloted++
    -6. patients[id].nurses_allotted
    -7. patients[id].num_nurses_allotted++
    -8. occupants[occ].nurses_alloted
    -9. occupants[occ].num_nurses_alloted++
    */
    
    //for (int i = 0; i < num_rooms; i++)
    //    for (int j = 0; j < days; j++) {
    //        free(room_schedule[i][j]);
    //        room_schedule[i][j] = NULL;  // Allocate space for days per room
    //        size_of_room_schedule[i][j] = 0;  // Allocate space for size tracking per day
    //    }
...
void orderCrossoverNurseGA(void) {
    ...
    bool* visited1 = (bool*)calloc(num_nurses, sizeof(bool));
    bool* visited2 = (bool*)calloc(num_nurses, sizeof(bool));
    if (!visited1 || !visited2) {
        printf("Memory allocation failed!\n");
...
void generateNewChromosomeNurseGA(int chromo_num)
{
    ...
    memcpy(NURSE_POPULATION[chromo_num], NURSE_POPULATION[chromo_num - 1], size * sizeof(int);
    for (j = 0; j < size; ++j) {
        do {
            r1 = rand() % (size);
            r2 = rand() % (size);
...
int main(void) {
    ...
    // printf("Mandatory Patients: %d\n", size);
    reset_valuesNewGA();
    append_optionalsNewGA();
    /*printf("\nG_BEST : \n");
    for (int i = 0; i < optional_count + mandatory_count + 2; i++) {
        printf("%d\t", G_BEST[i]);
    }*/
    //update_num_patients_info();
    ApplyNewHillClimbing();
