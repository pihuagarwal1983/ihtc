...
int partition_for_surgeons(Surgeon_data **s_data_arr, int n, int pivot) {
    int i, j;
    for (i=0, j=n-1; i<=j; ) {
        while (s_data_arr[i]->surgery_duration_sum >= pivot) ++i;
        while (s_data_arr[j]->surgery_duration_sum < pivot) --j;
        if (i <= j) swap_for_surgeons(s_data_arr, i++, j--);
    }
    return i;
}
...
void quick_sort_surgeons_on_s_duration_sum(Surgeon_data **s_data_arr, int n) {
    ...
    pivot = getPivot_for_surgeons(s_data_arr, n);
...
void sort_s_data_arr(Surgeon_data **s_data_arr, int n, Node *head) {
    ...
    quick_sort_surgeons_on_s_duration_sum(s_data_arr, n);  
...
// int getPivot_for_OTs(OTs **ot_data_arr, int n, int d) {
//     int t1 = n / 4, t2 = (3 * n) / 4, t3 = n / 2;
//     if (t1 < 0 || t1 >= n || t2 < 0 || t2 >= n || t3 < 0 || t3 >= n) {
//         fprintf(stderr, "Error: Indices out of bounds\n");
//         exit(EXIT_FAILURE);
//     }
//     if (ot_data_arr[t1] == NULL || ot_data_arr[t2] == NULL || ot_data_arr[t3] == NULL) {
//         fprintf(stderr, "Error: ot_data_arr contains NULL pointers\n");
//         exit(EXIT_FAILURE);
//     }
//     while (t1 < n && ot_data_arr[t1]->max_ot_time[d] == 0) ++t1;
//     while (t2 < n && ot_data_arr[t2]->max_ot_time[d] == 0) ++t2;
//     while (t3 < n && ot_data_arr[t3]->max_ot_time[d] == 0) ++t3;

//     int x = (ot_data_arr[t1]->max_ot_time[d] + ot_data_arr[t2]->max_ot_time[d] + ot_data_arr[t3]->max_ot_time[d]) / 3;
//   // printf("\n%d" , x);

//     return (ot_data_arr[t1]->max_ot_time[d] + ot_data_arr[t2]->max_ot_time[d] + ot_data_arr[t3]->max_ot_time[d]) / 3;
// }
...
int partition_for_OTs(OTs **ot_data_arr, int n, int pivot, int d) {
    //int i = 0, j = n - 1;
    // while (i <= j) {
    //     if (ot_data_arr[i] == NULL || ot_data_arr[j] == NULL) {
    //         fprintf(stderr, "Error: NULL pointers during partition\n");
    //         exit(EXIT_FAILURE);
    //     }
    //     while (i < n && ot_data_arr[i]->max_ot_time[d] >= pivot) ++i;
    //     while (j >= 0 && ot_data_arr[j]->max_ot_time[d] < pivot) --j;
    //     if (i <= j) swap_for_OTs(ot_data_arr, i++, j--);
    // }
    // return i;
    ...
    for (j = 0; j <= n - 1; j++) {
        ...
        if (ot_data_arr[j]->max_ot_time[d] > pivot) {
            i++;
            swap_for_OTs(&ot_data_arr[i], i , j) ;						 																					
        }
    }
    swap_for_OTs(&ot_data_arr[i], i , j) ;	
    return i + 1;	
...
void quick_sort_OTs_on_max_ot_time(OTs **ot_data_arr, int n, int d) {
    // QS for ot_data_arr on maximum_surgery_time
    if (n == 1) return;
    ...
    int n_left = partition_for_OTs(ot_data_arr, n, pivot, d);
    if (n_left > 0) quick_sort_OTs_on_max_ot_time(ot_data_arr, n_left, d);
    if (n - n_left > 0) quick_sort_OTs_on_max_ot_time(ot_data_arr + n_left, n - n_left, d);
}
...
void sort_ot_data_arr(OTs **ot_data_arr, int d) {
    ...
    quick_sort_OTs_on_max_ot_time(ot_data_arr, num_ots, d);
}
...
GenderRoom *removeAndAppendGenderRoom(int r_id, GenderRoom *src) {
    ...
    if (self == src) { // it's the very first room in the genderRoom array, so it'll be differently dealt with.
...
typedef struct optionals_PQ {
    HeapNode node;
    struct optionals_PQ *next;
} Mand_opt_PQ;
            ...
            if(!s_data_arr[i]->isNull) {
                ...
                for (j=0; j < s_data_arr[i]->num_assigned_patients; ++j) {
                    ...
                    */
                        ...
                        current_ot = admitOptionalFromPQ(pq, d, ot_data_arr, current_ot);
...
// -----------------------------------------------BELOW: FUNCTIONS FOR NURSE ASSIGNMENTS--------------------------------------------------
...
int main (void) {
    ...
    parse_json("C:/sant_saran/C in One/IHTC/New folder/sol_test09.json");
    PriorityQueue *pq;
